...
namespace super_toolbox
{
    public class LightvnExtractor : BaseExtractor
    {
                    ...
                    Parallel.ForEach(files, new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount * 2 }, filePath =>
                    {
                        ...
                        try
                        {
                            if (File.Exists(filePath))
                            {
                                ...
                                UpdateProgress(_processedFileCount);
                            }
                        }
                        catch (Exception ex)
                        {
        ...
        private void UpdateProgress(int processedCount)
        {
        }
        ...
        private string DetectFileExtension(byte[] fileData)
        {
            foreach (var entry in FileTypeSignatures)
            {
                ...
                for (int i = 0; i < signatures.Length; i++)
                {
                    ...
                    // Special offset for WAVEfmt chunk (starts at byte 8)
                    if (entry.Key == ".wav" && i == 1)
                        offset = 8;

                    // Special offset for MP4 ftyp box (starts after size field)
                    if (entry.Key == ".mp4" && i == 0)
                        offset = 4;

                    // Only check ftyp for MP4 files (since mvhd can be anywhere)
                    if (entry.Key == ".mp4" && i > 0)
                        continue;
        ...
        /// </summary>
        private void RepackVndat(string sourceFolder, string password)
        {
            ...
            {
                Console.WriteLine($"正在备份原始文件为 {Path.GetFileName(backupFile)}...");
            ...
            if (usePassword)
            {
                Console.WriteLine($"正在加密 {Path.GetFileName(outputFile)} 使用 `{password}` 作为密码...");
                zipStream.Password = password;
            }
            else
            {
                Console.WriteLine($"正在创建 {Path.GetFileName(sourceFolder)} 的临时副本以执行XOR加密...");
                ...
                foreach (string file in files)
                {
                    Console.WriteLine($"正在加密 {Path.GetRelativePath(sourceFolder, file)}...");
                    XOR(file);
                }
            }

            Console.WriteLine($"正在创建 {outputFile} 压缩包...");
